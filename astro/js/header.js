class p extends HTMLElement {
    ...
    connectedCallback() {
        let e = this.getAttribute('path'),
            o = this.getAttribute('nav-active')
        e == null && (e = ''),
            (this.innerHTML = `
	...
	`),
            o && document.getElementById(o).classList.add('active')
    }
}
window.customElements.define('custom-header', p)
const l = window.matchMedia('(max-width:1279px)'),
    i = () => {
        const a = document.querySelector('.overlay'),
            e = document.querySelectorAll(
                '.headerBancoppel--subnivel .li-nivel-uno'
            )
        if (l.matches === !0) {
            var o = document.querySelector('.headerBancoppel--subnivel')
            ...
            document
                .querySelector('.headerBancoppel--subnivel')
                .classList.remove('show'),
                a.classList.remove('open')
            for (const t of e)
                t.addEventListener('mouseenter', () => {
                    a.classList.add('open'), a.classList.remove('close')
                }),
...
const s = document.querySelectorAll(
    '.headerBancoppel--subnivel--uno .accordion-button'
)
for (const a of s)
    a.addEventListener('click', () => {
        let e = a.getAttribute('data-name')
        dataLayer.push({
            nd1: 'Menú superior',
            nd2: e,
            nd3: '',
...
const r = document.querySelector(
    '.headerBancoppel--sup--accesos a[data-name=Banca]'
)
...
const d = document.querySelector(
    '.headerBancoppel--sup--accesos a#link-sesion-nav'
)
...
const m = document.querySelectorAll('.headerBancoppel--sup--main a')
for (const a of m)
    a.addEventListener('click', () => {
        let e = a.getAttribute('data-name')
        dataLayer.push({
...
const u = document.querySelectorAll(
    '.headerBancoppel--sup--accesos a:not(#btnBanca, #link-sesion-nav)'
)
for (const a of u)
    a.addEventListener('click', () => {
        let e = a.getAttribute('data-name')
...
const h = document.querySelectorAll('.headerBancoppel .li-nivel-uno a')
for (const a of h)
    a.addEventListener('click', () => {
        let e = a.closest('.li-nivel-uno'),
            o = a.getAttribute('data-name')
        dataLayer.push({
            nd1: 'Menu superior',
            nd2: e.querySelector('span').innerText + ' - ' + o,
            nd3: '',
            nd4: '',
            event: 'clicMenuSuperiorGeneral'
        })
    })
const n = window.matchMedia('(max-width:1279px)'),
    c = () => {
        const a = document.querySelectorAll(
            '.headerBancoppel--subnivel--uno .accordion-button'
        )
        if (n.matches === !0)
            for (const e of a) e.setAttribute('data-bs-toggle', 'collapse')
        else if (n.matches === !1)
            for (const e of a) e.setAttribute('data-bs-toggle', '')
    }
n.addListener(c)
c()
